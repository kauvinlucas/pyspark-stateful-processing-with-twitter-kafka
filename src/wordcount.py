...
if len(arguments) > 2 or len(arguments) < 2:
    ...
    if __name__ == '__main__':
        ...
        wordcount = recent_tweets.withColumn('word', f.explode(f.split(f.col('tweet_text'), ' '))).groupBy('word').count().sort('count', ascending=False)
        query_console = wordcount.writeStream.format("console").outputMode("complete").start()
